...
export async function checkForUpdates() {
    changes = await Unwrap(YuricordNative.updater.getUpdates());
...
export async function update() {
    ...
    const res = await Unwrap(YuricordNative.updater.update());

    if (res) {
        isOutdated = false;
        if (!await Unwrap(YuricordNative.updater.rebuild()))
...
export const getRepo = () => Unwrap(YuricordNative.updater.getRepo());
