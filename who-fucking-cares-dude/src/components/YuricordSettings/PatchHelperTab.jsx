...
function ReplacementInput({ replacement, setReplacement, replacementError }) {
    ...
    React.useEffect(() => void (isFunc ? onChange(replacement) : setError(void 0)), [isFunc]);
    return (<>
            ...
            {!isFunc && (<div className="vc-text-selectable">
            ...
            }).map(([placeholder, desc]) => (<Forms.FormText key={placeholder}>
                            {Parser.parse("`" + placeholder + "`")}: {desc}
                        </Forms.FormText>))}
                </div>)}

            <Switch className={Margins.top8} value={isFunc} onChange={setIsFunc} note="'replacement' will be evaled if this is toggled" hideBorder={true}>
                Treat as Function
            </Switch>
        </>);
}
function FullPatchInput({ setFind, setParsedFind, setMatch, setReplacement }) {
    ...
    function update() {
        ...
        try {
            const parsed = (0, eval)(`([${fullPatch}][0])`);
            if (!parsed.find)
        ...
        return `
{
    ...
    return (<SettingsTab title="Patch Helper">
            ...
            <Forms.FormDivider />
            {module && (<ReplacementComponent module={module} match={match} replacement={replacement} setReplacementError={setReplacementError}/>)}
