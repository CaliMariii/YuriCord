...
import { Button, ChannelStore, Constants, GuildStore, IconUtils, MessageStore, Parser, PermissionsBits, PermissionStore, RestAPI, Text, UserStore } from "@webpack/common";
...
function MessageEmbedAccessory({ message }) {
    // @ts-ignore
    const embeddedBy = message.YuricordEmbeddedBy ?? [];
    const accessories = [];
...
function AutomodEmbedAccessory(props) {
    ...
    const { parse } = Parser;
    const [channelLabel, iconUrl] = getChannelLabelAndIconUrl(channel);
    return <AutoModEmbed channel={channel} childrenAccessories={<Text color="text-muted" variant="text-xs/medium" tag="span" className={`${EmbedClasses.embedAuthor} ${EmbedClasses.embedMargin}`}>
