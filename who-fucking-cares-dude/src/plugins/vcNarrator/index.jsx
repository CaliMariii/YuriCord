...
function speak(text, settings = Settings.plugins.VcNarrator) {
    if (!text)
        return;
...
function playSample(tempSettings, type) {
    ...
    speak(formatText(settings[type + "Message"], currentUser.username, "general", currentUser.globalName ?? currentUser.username, GuildMemberStore.getNick(myGuildId, currentUser.id) ?? currentUser.username), settings);
}
export default definePlugin({
    ...
    settingsAboutComponent({ tempSettings: s }) {
        ...
        const types = useMemo(() => Object.keys(Yuricord.Plugins.plugins.VcNarrator.options).filter(k => k.endsWith("Message")).map(k => k.slice(0, -7)), []);
        ...
        return (<Forms.FormSection>
                ...
                {hasEnglishVoices && (<>
                    ...
                    gap: "1rem",
                            ...
                            {types.map(t => (<Button key={t} onClick={() => playSample(s, t)}>
                                    {wordsToTitle([t])}
                                </Button>))}
                        </div>
                    </>)}
