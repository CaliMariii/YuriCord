...
export function TranslateIcon({ height = 24, width = 24, className }) {
...
export const TranslateChatBarIcon = ({ isMainChat }) => {
    ...
    if (!isMainChat || !showChatBarButton)
        return null;
    const toggle = () => {
        ...
        if (newState && settings.store.showAutoTranslateAlert !== false)
            Alerts.show({
                title: "Yuricord Auto-Translate Enabled",
                body: <>
                    <Forms.FormText>
    ...
    if (shouldShowTranslateEnabledTooltip && settings.store.showAutoTranslateTooltip)
        return (<Tooltip text="Auto Translate Enabled" forceOpen>
                {() => button}
            </Tooltip>);
    return button;
};
