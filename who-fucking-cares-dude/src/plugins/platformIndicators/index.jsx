...
function getBadges({ userId }) {
    ...
    return Object.entries(status).map(([platform, status]) => ({
        component: () => (<span className="vc-platform-indicator">
                <PlatformIcon key={platform} platform={platform} status={status} small={false}/>
            </span>),
        key: `vc-platform-indicator-${platform}`
    }));
}
const PlatformIndicator = ({ user, wantMargin = true, wantTopMargin = false, small = false }) => {
    if (!user || user.bot)
        return null;
    ensureOwnStatus(user);
    const status = PresenceStore.getState()?.clientStatuses?.[user.id];
...
const indicatorLocations = {
    list: {
        ...
        onEnable: () => addMemberListDecorator("platform-indicator", props => <ErrorBoundary noop>
    ...
    messages: {
        ...
        onEnable: () => addMessageDecoration("platform-indicator", props => <ErrorBoundary noop>
