...
export default definePlugin({
    ...
    patches: [
        {
            ...
            replacement: {
                match: /uploadFiles:(\i),/,
                replace: "uploadFiles:(...args)=>(args[0].uploads.forEach(f=>f.filename=$self.anonymise(f)),$1(...args)),",
            },
        },
        {
    ...
    settings,
    renderIcon: ErrorBoundary.wrap(({ upload, channelId, draftType }) => {
        const anonymise = upload.anonymise ?? settings.store.anonymiseByDefault;
        return (<ActionBarIcon tooltip={anonymise ? "Using anonymous file name" : "Using normal file name"} onClick={() => {
    ...
    anonymise(upload) {
        ...
        switch (settings.store.method) {
            case 0 /* Methods.Random */:
                ...
                return Array.from({ length: settings.store.randomisedLength }, () => chars[Math.floor(Math.random() * chars.length)]).join("") + ext;
