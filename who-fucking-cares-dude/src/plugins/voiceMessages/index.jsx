...
const ctxMenuPatch = (children, props) => {
    if (props.channel.guild_id && !(PermissionStore.can(PermissionsBits.SEND_VOICE_MESSAGES, props.channel) && PermissionStore.can(PermissionsBits.SEND_MESSAGES, props.channel)))
        return;
    children.push(<Menu.MenuItem id="vc-send-vmsg" label={<div className={OptionClasses.optionLabel}>
                ...
                </div>} action={() => openModal(modalProps => <Modal modalProps={modalProps}/>)}/>);
};
...
function Modal({ modalProps }) {
    ...
    const isUnsupportedFormat = blob && (!blob.type.startsWith("audio/ogg")
    ...
    return (<ModalRoot {...modalProps}>
            ...
            if (file) {
                ...
                {isUnsupportedFormat && (<Card className={`vc-plugins-restart-card ${Margins.top16}`}>
