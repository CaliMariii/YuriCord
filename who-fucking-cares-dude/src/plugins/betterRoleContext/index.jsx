    ...
    contextMenus: {
        "dev-context"(children, { id }) {
            ...
            if (!role)
                return;
            if (role.colorString) {
                children.unshift(<Menu.MenuItem id="vc-copy-role-color" label="Copy Role Color" action={() => Clipboard.copy(role.colorString)} icon={AppearanceIcon}/>);
            }
            ...
            if (role.icon) {
                children.push(<Menu.MenuItem id="vc-view-role-icon" label="View Role Icon" action={() => {
                        openImageModal({
