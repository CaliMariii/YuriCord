...
function CompactConnectionComponent({ connection, theme }) {
    ...
    return (<Tooltip text={<span className="vc-sc-tooltip">
            ...
            ? <a {...tooltipProps} className="vc-user-connection" href={url} target="_blank" rel="noreferrer" onClick={e => {
                    if (Yuricord.Plugins.isPluginEnabled("OpenInApp")) {
                        const OpenInApp = Yuricord.Plugins.plugins.OpenInApp;
            ...
            : <button {...tooltipProps} className="vc-user-connection" onClick={() => copyWithToast(connection.name)}>
                        {img}
                    </button>}
        </Tooltip>);
}
