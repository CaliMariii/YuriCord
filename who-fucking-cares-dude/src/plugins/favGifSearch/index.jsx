...
const containerClasses = findByPropsLazy("searchBar", "searchBarFullRow");
...
export default definePlugin({
    ...
    getTargetString,
    instance: null,
    renderSearchBar(instance, SearchBarComponent) {
    ...
    getFav(favorites) {
        if (!this.instance || this.instance.dead)
            return favorites;
...
function SearchBar({ instance, SearchBarComponent }) {
    ...
    return (<SearchBarComponent ref={ref} autoFocus={true} className={containerClasses.searchBar} size={SearchBarComponent.Sizes.MEDIUM} onChange={onChange} onClear={() => {
            setQuery("");
...
export function getTargetString(urlStr) {
    let url;
    try {
        url = new URL(urlStr);
    }
