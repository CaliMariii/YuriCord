...
function makeShortcuts() {
    function newFindWrapper(filterFactory) {
        const cache = new Map();
    ...
    return {
        ...
        preEnable: (plugin) => (Yuricord.Settings.plugins[plugin] ??= { enabled: true }).enabled = true,
        ...
        Stores: {
            getter: () => Object.fromEntries(Common.Flux.Store.getAll()
...
export default definePlugin({
    ...
    start() {
        ...
        Webpack.onceReady.then(() => {
            ...
            if (!IS_WEB) {
                const Native = YuricordNative.pluginHelpers.ConsoleShortcuts;
