            ...
            <ModalRoot {...props} size={"large" /* ModalSize.LARGE */}>
                ...
                <ModalFooter>
                    <Flex cellSpacing={10}>
                        ...
                        {!!msgContent && (<Button onClick={() => copyWithToast(msgContent, "Content copied to clipboard!")}>
                                Copy Raw Content
                            </Button>)}
                    </Flex>
                </ModalFooter>
...
function MakeContextCallback(name) {
    return (children, props) => {
        ...
        children.splice(-1, 0, <Menu.MenuItem id={`vc-view-${name.toLowerCase()}-raw`} label="View Raw" action={() => openViewRawModal(JSON.stringify(value, null, 4), name)} icon={CopyIcon}/>);
    };
}
const devContextCallback = (children, { id }) => {
    ...
    if (!role)
        return;
    children.push(<Menu.MenuItem id={"vc-view-role-raw"} label="View Raw" action={() => openViewRawModal(JSON.stringify(role, null, 4), "Role")} icon={CopyIcon}/>);
};
