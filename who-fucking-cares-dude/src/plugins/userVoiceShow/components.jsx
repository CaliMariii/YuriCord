...
function SpeakerIcon(props) {
    props.size ??= 16;
    return (<div {...props} role={props.onClick != null ? "button" : undefined} className={classes(cl("speaker"), props.onClick != null ? cl("clickable") : undefined, props.className)}>
...
function VoiceChannelTooltip({ channel, isLocked }) {
    ...
    return (<>
            {guild != null && (<div className={cl("name")}>
...
const clickTimers = {};
export const VoiceChannelIndicator = ErrorBoundary.wrap(({ userId, isMessageIndicator, isProfile, isActionButton, shouldHighlight }) => {
    ...
    if (!isDM && !PermissionStore.can(PermissionsBits.VIEW_CHANNEL, channel) && !Yuricord.Plugins.isPluginEnabled("ShowHiddenChannels"))
        return null;
