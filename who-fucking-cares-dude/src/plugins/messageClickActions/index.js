...
const keydown = (e) => e.key === "Backspace" && (isDeletePressed = true);
const keyup = (e) => e.key === "Backspace" && (isDeletePressed = false);
...
export default definePlugin({
    ...
    onMessageClick(msg, channel, event) {
        ...
        if (!isDeletePressed) {
            if (event.detail < 2)
                return;
            if (settings.store.requireModifier && !event.ctrlKey && !event.shiftKey)
                return;
            ...
            if (msg.deleted === true)
                return;
            if (isMe) {
                if (!settings.store.enableDoubleClickToEdit || EditStore.isEditing(channel.id, msg.id) || msg.state !== "SENT")
                    return;
                MessageActions.startEditMessage(channel.id, msg.id, msg.content);
                event.preventDefault();
            }
            else {
                if (!settings.store.enableDoubleClickToReply)
                    return;
                const EPHEMERAL = 64;
                if (msg.hasFlag(EPHEMERAL))
                    return;
                const isShiftPress = event.shiftKey && !settings.store.requireModifier;
                const NoReplyMention = Yuricord.Plugins.plugins.NoReplyMention;
