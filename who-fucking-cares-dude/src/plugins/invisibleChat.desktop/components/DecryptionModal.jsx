...
export function DecModal(props) {
    ...
    return (<ModalRoot {...props}>
            ...
            if (!toSend || !props?.message)
                return;
            // @ts-expect-error
            Yuricord.Plugins.plugins.InvisibleChat.buildEmbed(props?.message, toSend);
            props.onClose();
        }}>
                    Decrypt
                </Button>
                <Button color={Button.Colors.TRANSPARENT} look={Button.Looks.LINK} style={{ left: 15, position: "absolute" }} onClick={props.onClose}>
                    Cancel
                </Button>
            </ModalFooter>
        </ModalRoot>);
