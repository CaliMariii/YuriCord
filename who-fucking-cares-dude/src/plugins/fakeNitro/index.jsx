...
import { importApngJs } from "@utils/dependencies";
...
export default definePlugin({
    ...
    patches: [
        ...
        },
        {
            ...
            replacement: {
                ...
                replace: (_, reactNode, props) => `$self.addFakeNotice(${1 /* FakeNoticeType.Emoji */},${reactNode},!!${props}?.fakeNitroNode?.fake)`
            }
        },
        ...
        switch (embed.type) {
            case "image": {
                if (!settings.store.transformCompoundSentence
                    ...
                    return false;
                if (settings.store.transformEmojis) {
                    if (fakeNitroEmojiRegex.test(embed.url))
                        return true;
                }
                if (settings.store.transformStickers) {
                    if (fakeNitroStickerRegex.test(embed.url))
                        return true;
    ...
    filterAttachments(attachments) {
        return attachments.filter(attachment => {
            ...
            if (match) {
                // There is no way to differentiate a regular gif attachment from a fake nitro animated sticker, so we check if the StickerStore contains the id of the fake sticker
                if (StickerStore.getStickerById(match[1]))
                    return false;
            }
            return true;
        });
    ...
    start() {
        ...
        this.preSend = addMessagePreSendListener(async (channelId, messageObj, extra) => {
            ...
            stickerBypass: {
                ...
                if (sticker.format_type === 4 /* StickerType.GIF */ && link.includes(".png")) {
                    link = link.replace(".png", ".gif");
                }
                if (sticker.format_type === 2 /* StickerType.APNG */) {
