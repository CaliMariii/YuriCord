...
export default definePlugin({
    ...
    renderName: ErrorBoundary.wrap(({ session }) => {
        ...
        return (<>
                <span>{title}</span>
                {(savedSession == null || savedSession.isNew) && (<div className="vc-plugins-badge" style={{
    ...
    renderTimestamp: ErrorBoundary.wrap(({ session, timeLabel }) => {
    ...
    renderIcon: ErrorBoundary.wrap(({ session, DeviceIcon }) => {
        ...
        return (<BlobMask style={{ cursor: "unset" }} selected={false} lowerBadge={<div style={{
                    width: "20px",
                    height: "20px",
                    display: "flex",
                ...
                <div className={SessionIconClasses.sessionIcon} style={{ backgroundColor: GetOsColor(session.client_info.os) }}>
    ...
    flux: {
        USER_SETTINGS_ACCOUNT_RESET_AND_CLOSE_FORM() {
            const lastFetchedHashes = AuthSessionsStore.getSessions().map((session) => session.id_hash);
