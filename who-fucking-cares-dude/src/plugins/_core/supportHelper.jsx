...
async function generateDebugInfoMessage() {
    ...
    const info = {
        Yuricord: `v${VERSION} • [${gitHash}](<https://github.com/Vendicated/Yuricord/commit/${gitHash}>)` +
...
export default definePlugin({
    ...
    renderMessageAccessory(props) {
        ...
        if (shouldAddUpdateButton) {
            buttons.push(<Button key="vc-update" color={Button.Colors.GREEN} onClick={async () => {
                    try {
        ...
        if (props.channel.id === SUPPORT_CHANNEL_ID) {
            if (props.message.content.includes("/Yuricord-debug") || props.message.content.includes("/Yuricord-plugins")) {
                buttons.push(<Button key="vc-dbg" onClick={async () => sendMessage(props.channel.id, { content: await generateDebugInfoMessage() })}>
                        Run /Yuricord-debug
                    </Button>, <Button key="vc-plg-list" onClick={async () => sendMessage(props.channel.id, { content: generatePluginList() })}>
    ...
    renderContributorDmWarningCard: ErrorBoundary.wrap(({ channel }) => {
        ...
        if (!isPluginDev(userId))
            return null;
        if (RelationshipStore.isFriend(userId) || isPluginDev(UserStore.getCurrentUser()?.id))
            return null;
        return (<Card className={`vc-plugins-restart-card ${Margins.top8}`}>
