...
function makeRenderMoreUsers(users) {
    return function renderMoreUsers(_label, _count) {
        return (<Tooltip text={users.slice(4).map(u => u.username).join(", ")}>
...
export default definePlugin({
    ...
    renderUsers(props) {
        return props.message.reactions.length > 10 ? null : (<ErrorBoundary noop>
                <this.UsersComponent {...props}/>
            </ErrorBoundary>);
    },
    UsersComponent({ message, emoji, type }) {
        ...
        return (<div style={{ marginLeft: "0.5em", transform: "scale(0.9)" }}>
                <div onClick={handleClickAvatar} onKeyPress={handleClickAvatar}>
                    <UserSummaryItem users={users} guildId={ChannelStore.getChannel(message.channel_id)?.guild_id} renderIcon={false} max={5} showDefaultAvatarsForNullUsers showUserPopout renderMoreUsers={makeRenderMoreUsers(users)}/>
                </div>
            </div>);
    },
    set reactions(value) {
