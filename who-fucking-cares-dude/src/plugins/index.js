...
export function isPluginEnabled(p) {
    return (Plugins[p]?.required ||
        Plugins[p]?.isDependency ||
        settings[p]?.enabled) ?? false;
}
export function addPatch(newPatch, pluginName) {
...
export const startAllPlugins = traceFunction("startAllPlugins", function startAllPlugins(target) {
    ...
    if (p.flux && !subscribedFluxEventsPlugins.has(p.name) && (!IS_REPORTER || isReporterTestable(p, 16 /* ReporterTestable.FluxEvents */))) {
        ...
        for (const [event, handler] of Object.entries(p.flux)) {
            const wrappedHandler = p.flux[event] = function () {
                try {
                    const res = handler.apply(p, arguments);
...
export const startPlugin = traceFunction("startPlugin", function startPlugin(p) {
    const { name, commands, contextMenus, userProfileBadge, onBeforeMessageEdit, onBeforeMessageSend, onMessageClick, renderChatBarButton, renderMemberListDecorator, renderMessageAccessory, renderMessageDecoration, renderMessagePopoverButton } = p;
    if (p.start) {
    ...
    if (renderChatBarButton)
        addChatBarButton(name, renderChatBarButton);
