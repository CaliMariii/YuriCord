...
export const createHighlighter = (props) => (<pre className={cl("container")}>
        <ErrorBoundary>
            <Highlighter {...props}/>
        </ErrorBoundary>
    </pre>);
export const Highlighter = ({ lang, content, isPreview, tempSettings, }) => {
    const { tryHljs, useDevIcon, bgOpacity, } = useShikiSettings(["tryHljs", "useDevIcon", "bgOpacity"], tempSettings);
    ...
    const themeBase = {
        ...
        backgroundColor: currentTheme?.colors?.["editor.background"] || "var(--background-secondary)",
    };
    let langName;
    if (lang)
