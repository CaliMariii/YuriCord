...
const cl = (className) => `vc-spotify-${className}`;
...
function Svg(path, label) {
    return () => (<svg className={classes(cl("button-icon"), cl(label))} height="24" width="24" viewBox="0 0 24 24" fill="currentColor" aria-label={label} focusable={false}>
            <path d={path}/>
        </svg>);
}
...
function CopyContextMenu({ name, path }) {
    ...
    return (<Menu.Menu navId={`spotify-${name}-menu`} onClose={() => FluxDispatcher.dispatch({ type: "CONTEXT_MENU_CLOSE" })} aria-label={`Spotify ${name} Menu`}>
            <Menu.MenuItem key={copyId} id={copyId} label={`Copy ${name} Link`} action={() => copyWithToast("https://open.spotify.com" + path)} icon={LinkIcon}/>
            <Menu.MenuItem key={openId} id={openId} label={`Open ${name} in Spotify`} action={() => SpotifyStore.openExternal(path)} icon={OpenExternalIcon}/>
        </Menu.Menu>);
}
...
function Controls() {
    ...
    return (<Flex className={cl("button-row")} style={{ gap: 0 }}>
            ...
            <Button className={classes(cl("button"), cl(repeatClassName))} onClick={() => SpotifyStore.setRepeat(nextRepeat)} style={{ position: "relative" }}>
...
function SeekBar() {
    ...
    return (<div id={cl("progress-bar")}>
            <Forms.FormText variant="text-xs/medium" className={cl("progress-time") + " " + cl("time-left")} aria-label="Progress">
                {msToHuman(position)}
            </Forms.FormText>
            <Menu.MenuSliderControl minValue={0} maxValue={duration} value={position} onChange={(v) => {
            if (isSettingPosition)
                return;
            setPosition(v);
            seek(v);
        }} renderValue={msToHuman}/>
            <Forms.FormText variant="text-xs/medium" className={cl("progress-time") + " " + cl("time-right")} aria-label="Total Duration">
                {msToHuman(duration)}
            </Forms.FormText>
        </div>);
}
...
function Info({ track }) {
    ...
    return (<div id={cl("info-wrapper")}>
            {i}
            <div id={cl("titles")}>
                <Forms.FormText variant="text-sm/semibold" id={cl("song-title")} className={cl("ellipoverflow")} title={track.name} {...makeLinkProps("Song", track.id, `/track/${track.id}`)}>
                    {track.name}
                </Forms.FormText>
                {track.artists.some(a => a.name) && (<Forms.FormText variant="text-sm/normal" className={cl("ellipoverflow")}>
                        by&nbsp;
                        {track.artists.map((a, i) => (<React.Fragment key={a.name}>
                                <span className={cl("artist")} style={{ fontSize: "inherit" }} title={a.name} {...makeLinkProps("Artist", a.id, `/artist/${a.id}`)}>
                                    {a.name}
                                </span>
                                {i !== track.artists.length - 1 && <span className={cl("comma")}>{", "}</span>}
                            </React.Fragment>))}
                    </Forms.FormText>)}
                {track.album.name && (<Forms.FormText variant="text-sm/normal" className={cl("ellipoverflow")}>
                        on&nbsp;
                        <span id={cl("album-title")} className={cl("album")} style={{ fontSize: "inherit" }} title={track.album.name} {...makeLinkProps("Album", track.album.id, `/album/${track.album.id}`)}>
                            {track.album.name}
                        </span>
                    </Forms.FormText>)}
            </div>
