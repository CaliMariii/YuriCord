...
import { openNotificationSettingsModal } from "@components/YuricordSettings/NotificationSettings";
...
export async function deleteNotification(timestamp) {
...
function LogModal({ modalProps, close }) {
    ...
    return (<ModalRoot {...modalProps} size={"large" /* ModalSize.LARGE */}>
            ...
            <ModalFooter>
                <Flex>
                    ...
                    <Button disabled={log.length === 0} color={Button.Colors.RED} onClick={() => {
            Alerts.show({
                title: "Are you sure?",
...
export function openNotificationLogModal() {
    const key = openModal(modalProps => (<LogModal modalProps={modalProps} close={() => closeModal(key)}/>));
}
